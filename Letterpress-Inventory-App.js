...
const ALL_STAT_TILES = [
  ...
  { id: 'stat-machines', icon: '🎰', color: 'coral', label: 'Active Machines' },
];
  ...
  container.innerHTML = `<div class="items-grid">${items.map(item => `
    <div class="item-card ${item.Status === 'Retired' ? 'item-retired' : ''}" onclick="openItemDetail('${item.ItemID}')">
      <div class="item-card-image">
        ${item.Photo
          ? `<img src="${item.Photo}" alt="${item.DisplayName || item.Name}" loading="lazy">`
      ...
      <table class="items-table">
        ...
        <tbody>${items.map(item => `
          <tr onclick="openItemDetail('${item.ItemID}')">
            <td><div class="table-thumbnail">${item.Photo ? `<img src="${item.Photo}" alt="" style="width:100%;height:100%;object-fit:cover;">` : '<svg width="22" height="22" viewBox="0 0 36 36" fill="none"><rect x="3" y="6" width="30" height="24" rx="3" stroke="#C4A882" stroke-width="2"/></svg>'}</div></td>
...
function openItemDetail(itemId) {
  ...
  openDetailPanel(`
    ...
    <div class="detail-image">
      ${item.Photo ? `<img src="${item.Photo}" alt="${item.DisplayName}">` : `<div class="detail-image-placeholder">${dogEmpty()}</div>`}
    </div>
  ...
  openModal(`
    ...
    <form id="edit-item-form">
      ...
      <div class="form-field">
        ...
        ${item.Photo ? `<div class="photo-preview-wrap"><img class="photo-preview-thumb" src="${item.Photo}" alt="Current photo"></div>` : ''}
...
function renderPartnersGrid(partners) {
  ...
  container.innerHTML = `<div class="partners-grid">${partners.map(p => `
    <div class="partner-card" onclick="openPartnerDetail('${p.value}')">
      <div class="partner-card-image">${p.storePhoto ? `<img src="${p.storePhoto}" alt="${p.label}">` : '🏪'}</div>
...
function renderMachinesSummary(machines) {
  ...
  bar.innerHTML = `
    <div class="machines-stats">
      <div class="machine-stat-pill teal">🎰 ${deployed.length} Deployed</div>
  ...
  container.innerHTML = `<div class="partners-grid">${machines.map(m => {
    ...
    return `
      <div class="partner-card" onclick="openMachineDetail('${m.MachineID}')">
        <div class="partner-card-image" style="background:var(--cream-dark);font-size:3rem;">🎰</div>
        <div class="partner-card-body">
...
function openMachineDetail(machineId) {
  ...
  openDetailPanel(`
    ...
    <div style="font-size:5rem;text-align:center;margin:1rem 0;">🎰</div>
    ${removalAlert}
    <div class="detail-stats">
...
function openAddMachineModal() {
  openModal(`
    <div class="modal-title">🎰 Add Vending Machine</div>
